/**
 * Code Interpreter Tool — Sandboxed JavaScript Execution
 *
...
export function executeCode(code: string, language = 'javascript'): CodeInterpreterResult {
  ...
  // Build sandboxed context with safe globals only
  ...
  try {
    ...
    // Include any logs captured before the error
    const outputParts: string[] = []
    if (logs.length > 0) {
      outputParts.push(logs.join('\n'))
    }
...
/**
 * Agent tool wrapper — called from AgentLoop.executeTool()
 */
export async function toolCodeInterpreter(args: Record<string, unknown>): Promise<string> {
  ...
  // Handle language check before executeCode (which returns empty output for unsupported)
  if (language !== 'javascript' && language !== 'js') {
  ...
  if (result.success) {
    ...
    if (!result.output && result.returnValue === undefined) {
      parts.push('\n(no output)')
    }
    return parts.join('')
  }
