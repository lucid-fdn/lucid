-- ============================================================================
-- Migration 048 v3: Memory User-Scoped Deduplication (PRODUCTION GRADE)
-- ============================================================================
-- 
...
-- 
-- FIX (v3 - Production Grade):
-- 1. Add external_user_id + scoped_user_id columns (don't exist yet!)
...
-- 
-- Changes from v2 → v3:
-- - search_memory(): p_scoped_user_id now REQUIRED (throws error if NULL)
-- - Retrieval: Explicitly filters WHERE scoped_user_id IS NOT NULL
-- - Added: Functional tests (two users, same content → must create two rows)
-- - Added: Retrieval isolation test (User B must not see User A's memories)
-- 
-- Priority: 🔴 CRITICAL - Confirmed Data Leakage + Security Hole in v2
-- ============================================================================
...
--
-- AFTER (Migration 048 v3 - PRODUCTION GRADE):
...
--
-- NEXT STEPS:
-- 1. Deploy this migration (adds columns + indexes + functions + tests)
...
-- 5. Review legacy memories (optional - decide on backfill vs delete)
--
-- ============================================================================