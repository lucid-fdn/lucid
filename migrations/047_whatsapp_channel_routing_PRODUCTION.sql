-- ============================================================================
-- Migration 047: WhatsApp Channel-Based Routing (PRODUCTION-READY v3)
-- ============================================================================
-- 
...
-- 
-- Dev Review Fixes Applied (ALL 3 CRITICAL FIXES):
-- ✅ Fix #1: Idempotent constraint creation (DO block)
-- ✅ Fix #2: SHA-256 instead of MD5 + guard COMMENT ON COLUMN
...
-- ----------------------------------------------------------------------------
-- 2) Routing constraints and indexes (FIX #1: Idempotent constraint)
-- ----------------------------------------------------------------------------

-- Ensure external_channel_id is NOT NULL for WhatsApp channels
-- FIX #1: Wrap in DO block for idempotency (constraint has no IF NOT EXISTS)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
...
-- ----------------------------------------------------------------------------
-- 5) Webhook idempotency index (NULL-safe)
-- ----------------------------------------------------------------------------
...
--
-- Dev Review Fixes Applied:
-- ✅ Fix #1: Idempotent constraint creation (DO block checks pg_constraint)
-- ✅ Fix #2: SHA-256 instead of MD5 + guarded COMMENT ON COLUMN
