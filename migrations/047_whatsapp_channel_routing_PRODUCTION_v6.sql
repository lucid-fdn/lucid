-- ============================================================================
-- Migration 047: WhatsApp Channel-Based Routing (PRODUCTION-READY v6)
-- ============================================================================
-- 
...
-- 
-- Dev Review Fixes Applied (ALL 8 CRITICAL FIXES):
...
-- ✅ Fix #7: DROP + recreate WhatsApp index (same pattern as Fix #5)
-- ✅ Fix #8: Improved duplicate cleanup (tie-breaker + only active duplicates)
...
-- FIX #6 + FIX #8: Preflight check for duplicate WhatsApp channels before creating UNIQUE index
DO $$
DECLARE
  v_duplicate_count INTEGER;
BEGIN
  -- Check for duplicate external_channel_id in ACTIVE WhatsApp channels
  ...
  FROM (
    ...
    WHERE channel_type = 'whatsapp'
      ...
      AND is_active = true  -- FIX #8: Only check active channels
    GROUP BY external_channel_id
    HAVING COUNT(*) > 1
  ) duplicates;
  
  IF v_duplicate_count > 0 THEN
    -- FIX #8: Improved cleanup with tie-breaker + only deactivate active duplicates
    -- Cleanup strategy: Deactivate all but the oldest ACTIVE channel per phone_number_id
    -- Uses (created_at, id) for deterministic ordering (tie-breaker when timestamps equal)
    ...
    WHERE id IN (
      ...
      INNER JOIN assistant_channels c2
        ...
        AND c1.is_active = true  -- FIX #8: Only consider active channels for "oldest"
        AND c2.is_active = true  -- FIX #8: Only deactivate active duplicates
        -- Keep oldest by created_at, use id as tie-breaker (deterministic when timestamps equal)
        AND (c1.created_at < c2.created_at OR (c1.created_at = c2.created_at AND c1.id < c2.id))
    );
    
    RAISE NOTICE 'Found % duplicate active WhatsApp channels. Deactivated newer duplicates (kept oldest per phone_number_id with tie-breaker).', v_duplicate_count;
  END IF;
END $$;

-- FIX #7: DROP + recreate WhatsApp index (same pattern as Fix #5)
-- IF NOT EXISTS won't replace old index with different predicate
...
BEGIN
  ...
  WHERE c.channel_type = 'whatsapp'
    ...
    AND c.is_active = true  -- SANITY CHECK: Only route to active channels
  LIMIT 1;
END;
$$;

COMMENT ON FUNCTION get_channel_by_whatsapp_phone_number_id IS 
  'Routes WhatsApp webhook by phone_number_id → channel. Worker decrypts access token from encrypted_secrets using encrypted_secrets_id. Filters by is_active=true to prevent routing to inactive channels.';
...
-- - FIX #5: DROP + recreate Telegram dedupe index (IF NOT EXISTS won't replace broken index)
-- - FIX #7: DROP + recreate WhatsApp index (same migration-mechanics fix)
...
--
-- WhatsApp Channel Uniqueness (FIX #6 + FIX #8):
-- - Preflight check for duplicate external_channel_id before creating UNIQUE index
-- - FIX #8: Only checks/deactivates ACTIVE duplicates (ignores inactive historical ones)
-- - FIX #8: Uses (created_at, id) tie-breaker for deterministic ordering when timestamps equal
...
--
-- Routing Safety (SANITY CHECK):
-- - get_channel_by_whatsapp_phone_number_id() filters by is_active=true
-- - Prevents routing to inactive/deactivated channels
-- - Ensures UNIQUE index constraint matches routing logic
--
-- Dev Review Fixes Applied (ALL 8):
...
-- ✅ Fix #5: DROP + recreate Telegram dedupe index (IF NOT EXISTS won't replace) + WHERE BOTH NOT NULL
-- ✅ Fix #6: Preflight duplicate cleanup (prevents UNIQUE index creation failure)
-- ✅ Fix #7: DROP + recreate WhatsApp index (same migration-mechanics fix as #5)
-- ✅ Fix #8: Improved duplicate cleanup (tie-breaker for created_at + only active duplicates)