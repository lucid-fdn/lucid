-- ============================================================================
-- Migration 047: WhatsApp Channel-Based Routing (PRODUCTION-READY v4)
-- ============================================================================
-- 
...
-- 
-- Dev Review Fixes Applied (ALL 4 CRITICAL FIXES):
...
-- ----------------------------------------------------------------------------
-- 5) Webhook idempotency index (FIX #4: CRITICAL - Telegram collision fix)
-- ----------------------------------------------------------------------------
...
--
-- UNIQUE partial index: guards against NULL
...
COMMENT ON INDEX ux_inbound_webhook_dedupe IS 
  'Prevents duplicate webhook processing. Includes external_chat_id for Telegram (message_id is only unique per chat). WhatsApp message IDs are globally unique so this is safe for both. Partial index excludes NULL (status updates, etc.). Webhook insert must use ON CONFLICT DO NOTHING.';
...
-- 
-- Idempotency:
-- - Webhook: Unique partial index on (channel_id, external_chat_id, external_message_id WHERE NOT NULL)
...
--
-- Dev Review Fixes Applied (ALL 4):
