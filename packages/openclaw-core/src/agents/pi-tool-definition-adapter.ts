...
// When an executor is provided, tools execute inline and return real results.
// Without an executor, tools return "pending" (original OpenResponses pattern).
  ...
  return tools.map((tool) => {
    ...
    return {
      ...
      execute: async (...args: ToolExecuteArgs): Promise<AgentToolResult<unknown>> => {
        ...
        // If executor provided, execute inline and return real result
        if (executor) {
          const result = await executor(func.name, paramsRecord);
