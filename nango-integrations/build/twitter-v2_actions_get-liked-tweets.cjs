...
// twitter-v2/actions/get-liked-tweets.ts
var get_liked_tweets_exports = {};
__export(get_liked_tweets_exports, {
  default: () => get_liked_tweets_default
});
module.exports = __toCommonJS(get_liked_tweets_exports);
...
var inputSchema = z.object({
  ...
  max_results: z.number().min(5).max(100).optional().describe("Number of results (5-100, default 20)")
});
...
var action = {
  ...
  exec: async (nango, input) => {
    ...
    return {
      ...
      result_count: response.data?.meta?.result_count ?? tweets.length,
      next_token: response.data?.meta?.next_token
    };
  }
};
var get_liked_tweets_default = action;
