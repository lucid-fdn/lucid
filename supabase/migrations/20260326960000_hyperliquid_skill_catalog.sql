-- Seed Hyperliquid perpetuals trading skill into skill_catalog (DB-managed).
-- Source: worker/src/skills/hyperliquid/SKILL.md
-- Auto-install + auto-activate for existing trading-enabled assistants.
-- Capability gating: execute:perpetuals (tools are built-in, not plugin-activated).
-- OpenClaw-compatible: SKILL.md uses standard frontmatter, no platform-specific coupling.

-- 1. Seed into skill_catalog
INSERT INTO skill_catalog (
  slug, name, description, raw_content, sanitized_content, frontmatter,
  source, content_hash, status, content_chars, approved_at
) VALUES (
  'hyperliquid',
  'Hyperliquid Perpetuals Trading Guide',
  'Agent guide for Hyperliquid perpetual futures — margin mechanics, leverage, liquidation, order types, position management, funding rates, risk controls, and failure recovery',
  E'
# Hyperliquid Perpetuals Trading Guide

You have access to hl_account_info (read-only), hl_place_order (elevated), and hl_cancel_order (elevated) for trading perpetual futures on Hyperliquid.
...
**Signing**: EIP-712 typed data (order and cancel actions must be signed by the trading wallet)
...
- When showing multiple positions, use a table format for clarity
',
  '{"category":"trading","version":"1.0","author":"Lucid"}'::jsonb,
  'manual',
  md5('hyperliquid-skill-v1.2'),
  'approved',
  14939,
  now()
...
-- 2. Auto-install for existing wallet-enabled orgs (wallet_enabled is the proxy for trading orgs)
...
WHERE aa.wallet_enabled = true AND aa.org_id IS NOT NULL
  AND sc.slug = 'hyperliquid'
