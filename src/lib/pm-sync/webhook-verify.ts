/**
 * Webhook Signature Verification — Crypto primitives shared by all adapters.
 *
 * Adapters call these helpers from their per-provider `verifySignature()`
 * implementation. The helpers themselves know nothing about any specific
 * provider — they just compute HMACs and compare in constant time.
 *
 ...
 * Linear sends plain hex, GitHub-style `sha256=<hex>`, Asana sends
 * `X-Hook-Signature: <hex>` — this helper gracefully handles all of them.
 *
