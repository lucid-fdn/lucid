...
function getSecret(secret?: string): Buffer {
  const key = secret ?? process.env.SLACK_HOSTED_STATE_SECRET
  if (!key || key.length < 32) {
    throw new Error('SLACK_HOSTED_STATE_SECRET is missing or too short (min 32 chars).')
  }
...
export function issueSlackOAuthState(input: {
  assistantId: string
  orgId: string
  userId: string
  secret?: string
