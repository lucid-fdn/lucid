...
import { postOpenAiCompatibleAudioForm } from '@/lib/media/openai-compatible-audio'

export { normalizeProviderBaseUrl, normalizeProviderSecret, uniqueDefined } from '@/lib/ai/provider-policy'
...
export interface AudioTranscriptionResult {
  text: string
  provider: Exclude<AudioTranscriptionProvider, 'auto'>
  model: string
  inputBytes: number
  mimeType: string
  fileName: string
...
export function buildAudioTranscriptionCandidates(params: {
  ...
  const pushTrustGate = () => {
    for (const baseUrl of params.gatewayBaseUrls) {
      for (const apiKey of params.gatewayApiKeys) {
        pushOpenAiCompatible('trustgate', baseUrl, apiKey, DEFAULT_TRANSCRIPTION_MODEL)
      }
    }
  }

  const pushOpenAi = () => pushOpenAiCompatible(
    'openai',
  ...
  const pushGroq = () => pushOpenAiCompatible(
    'groq',
  ...
  const pushMistral = () => pushOpenAiCompatible(
    'mistral',
...
export async function transcribeAudioWithCandidatesDetailed(params: {
  buffer: Buffer
  mimeType: string
  fileName: string
  ...
  for (const candidate of params.candidates) {
    try {
      ...
      const transcript = candidate.kind === 'deepgram'
      ...
      return {
        ...
        model: candidate.model,
        inputBytes: params.buffer.byteLength,
        mimeType: params.mimeType,
        fileName: params.fileName,
...
export async function transcribeAudioWithCandidates(params: {
  buffer: Buffer
  mimeType: string
  fileName: string
  candidates: AudioTranscriptionCandidate[]
}): Promise<string> {
  const result = await transcribeAudioWithCandidatesDetailed(params)
  return result.text
}
