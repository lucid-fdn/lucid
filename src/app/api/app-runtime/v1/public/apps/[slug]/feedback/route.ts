...
import { submitPublicFeedback } from '@/lib/app-service/runtime-gateway/public'
import {
  publicRuntimeAccess,
...
export async function POST(
  ...
  const access = publicRuntimeAccess(request, 'feedback')
  const reqId = requestIdFromRequest(request)
  try {
    requirePublicRuntimeSurfaces()
    const { slug } = await params
    const body = await readRuntimeJsonBody(request)
    const feedback = await submitPublicFeedback(slug, body, access)
    return publicRuntimeOk({ feedback }, reqId, access, { status: 202 })
  } catch (error) {
