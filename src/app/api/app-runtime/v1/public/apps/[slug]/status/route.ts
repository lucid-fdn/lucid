...
import { getPublicAppStatus } from '@/lib/app-service/runtime-gateway/public'
import {
  publicRuntimeAccess,
...
export async function GET(
  ...
  const access = publicRuntimeAccess(request, 'status', false)
  const reqId = requestIdFromRequest(request)
  try {
    requirePublicRuntimeSurfaces()
    const { slug } = await params
    return publicRuntimeOk(await getPublicAppStatus(slug, access), reqId, access)
  } catch (error) {
