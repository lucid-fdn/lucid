...
export async function POST(
  ...
  try {
      ...
      return NextResponse.json(
        {
          error:
            result.existingAssistantId === id
              ...
              : `"${parsed.data.alias}" is already used by another agent in this iMessage chat.`,
    ...
    ErrorService.captureException(error as Error, {
      severity: 'error',
      context: { endpoint: '/api/assistants/[id]/imessage-aliases', method: 'POST' },
      tags: { layer: 'api', route: 'assistant-imessage-aliases' },
    })
    return NextResponse.json({ error: 'Failed to create iMessage alias.' }, { status: 500 })
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (!deleted) {
      return NextResponse.json({ error: 'iMessage alias not found.' }, { status: 404 })
    }
    ...
    ErrorService.captureException(error as Error, {
      severity: 'error',
      context: { endpoint: '/api/assistants/[id]/imessage-aliases', method: 'DELETE' },
      tags: { layer: 'api', route: 'assistant-imessage-aliases' },
    })
    return NextResponse.json({ error: 'Failed to delete iMessage alias.' }, { status: 500 })
  }
}
