...
import { KnowledgeImportCommitRequestSchema } from '@contracts/knowledge-imports'
...
export const POST = withCSRF(async (
  ...
  try {
    ...
    const body = KnowledgeImportCommitRequestSchema.parse(await req.json())
    ...
    ErrorService.captureException(error as Error, {
      severity: 'error',
      context: { endpoint: '/api/knowledge/imports/[id]/commit', method: 'POST' },
      tags: { layer: 'api', route: 'knowledge-imports' },
    })
    return NextResponse.json({ error: 'Failed to commit import job' }, { status: 500 })
  }
})
