...
export const POST = withCSRF(async (req: NextRequest) => {
  try {
    ...
    const response = await fetch(`${workerUrl}/trigger`, {
      ...
      body: JSON.stringify({
        event_type: 'knowledge_brain_ops',
        org_id: body.org_id,
      }),
    })
    ...
    ErrorService.captureException(error as Error, {
      severity: 'error',
      context: { endpoint: '/api/knowledge/maintenance/run', method: 'POST' },
      tags: { layer: 'api', route: 'knowledge' },
    })
    return NextResponse.json({ error: 'Failed to trigger Knowledge Brain Ops' }, { status: 500 })
  }
})
