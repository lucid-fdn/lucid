...
import { pricingTable, tiers, TierName } from "@/components/marketing/lucid/constants/pricing";
...
export const PricingTable = () => {
  ...
  const titleToPrice: Record<
    string,
    { monthly: number; yearly: number; ctaText: string; ctaLink: string }
  > =
    useMemo(() => {
      const map: Record<
        string,
                ...
                {orderedTierNames.map((tierName) => (
                  ...
                  >
                    ...
                    <div className="flex items-center text-sm font-normal text-gray-600 dark:text-neutral-300">
                      {tierName === TierName.TIER_3 && cycle === "yearly" ? (
                        "Talk to sales"
                      ) : (
                        <>
                    ...
                    <Button
                      as={Link}
                      href={titleToPrice[tierName]?.ctaLink || "/sign-up"}
                      className="mt-4 w-full"
                      variant="secondary"
                    >
                      {titleToPrice[tierName]?.ctaText || "Start for free"}
                    </Button>
                  </th>
                ))}
              </tr>
